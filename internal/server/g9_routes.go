...
func RegisterG9Routes(mux *http.ServeMux, store *db.LocalDB) {
	...
	mux.HandleFunc("/api/g9/mix-leaderboard", handleG9MixLeaderboard(store))
	mux.HandleFunc("/api/g9/rework", handleG9Rework(store))
	...
	mux.HandleFunc("/api/g9/engineer/", handleG9EngineerDetail(store))
}
